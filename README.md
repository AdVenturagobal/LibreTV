...
6. （可选）绑定 KV 命名空间 `LIBRETV_PROXY_KV` 以启用 M3U8 缓存
7. 点击"保存并部署"

> **提示**：如果你更习惯使用 Cloudflare Workers，可以直接运行 `npm run deploy:cf`（内部调用 `wrangler deploy`）。相关 Worker 配置见 `wrangler.toml` 与 `worker.js`。
